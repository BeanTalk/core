...
public class MenuDao extends BaseDao<Menu> {
	...
	public List<Menu> findByUserId(String userId) {
		return find(
				"select distinct m from Menu m, Role r, User u where m in elements (r.menuList) and r in elements (u.roleList)"
						+ " and m.delFlag=:p1 and r.delFlag=:p1 and u.delFlag=:p1 and u.id=:p2" + " order by m.sort",
	...
	public List<Menu> findAllActivitiList(String userId) {
		return find(
				"select distinct m from Menu m, Role r, User u where m in elements (r.menuList) and r in elements (u.roleList)"
						+ " and m.delFlag=:p1 and r.delFlag=:p1 and u.delFlag=:p1 and u.id=:p32order by m.sort",
				new Parameter(Menu.DEL_FLAG_NORMAL, userId));
	}
